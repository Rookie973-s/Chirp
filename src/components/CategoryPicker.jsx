            ...
            {items.map(it=>{
              ...
              return (
                <button key={key} className={`cat-item${isOn?" on":""}`}
                  ...
                  {it.count&&<span style={{fontSize:11,background:isOn?"rgba(59,130,246,.18)":"var(--surfA)",padding:"2px 8px",borderRadius:8,color:isOn?T.gold:"var(--faint)",fontWeight:500}}>{it.count}</span>}
                </button>
              );
            })}
          </div>
