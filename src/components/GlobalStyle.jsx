// Inject once at the root — <GlobalStyle /> inside <App />
const GlobalStyle = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,600;0,700;1,600&family=DM+Sans:ital,wght@0,400;0,500;0,600;0,700;1,400&display=swap');
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    ::selection{background:#D4AF37;color:#061220}
    :focus-visible{outline:2px solid #D4AF37;outline-offset:2px}

    .light{--bg:#F8FAFC;--surf:#FFFFFF;--surfH:#F1F5F9;--surfA:#F3F6F9;--txt:#0F172A;--mut:#475569;--faint:#94A3B8;--brd:#E2E8F0;--brdS:#CBD5E1}
    .dark{--bg:#0A0F1A;--surf:#121B2B;--surfH:#1A2639;--surfA:#0D1420;--txt:#F1F5F9;--mut:#94A3B8;--faint:#475569;--brd:#1E293B;--brdS:#334155}
    ...
    @keyframes glow{0%,100%{box-shadow:0 0 0 0 rgba(212,175,55,.4)}50%{box-shadow:0 0 0 8px rgba(212,175,55,0)}}
    ...
    .btn-pri{background:#D4AF37;color:#061220}
    .btn-pri:hover{background:#F3E5AB;transform:translateY(-2px);box-shadow:0 6px 20px rgba(212,175,55,.35)}
    .btn-navy{background:#0B1F35;color:#fff}
    .btn-navy:hover{background:#1A365D;transform:translateY(-2px)}
    ...
    .inp:focus{background:var(--surf);border-color:#D4AF37;box-shadow:0 0 0 4px rgba(212,175,55,.15);outline:none}
    ...
    .tag:hover{background:rgba(212,175,55,.15);color:#D4AF37}

    /* Modal */
    .overlay{position:fixed;inset:0;background:rgba(6,18,32,.6);backdrop-filter:blur(8px);z-index:2000;display:flex;align-items:center;justify-content:center;padding:clamp(0px,4vw,24px);animation:fadeUp .2s ease}
    .modal{background:var(--surf);width:100%;border-radius:20px;box-shadow:0 25px 60px rgba(0,0,0,.25);overflow:hidden;animation:popIn .35s cubic-bezier(.34,1.56,.64,1);border:1px solid rgba(255,255,255,.1);max-height:min(90vh,calc(100dvh - 40px));overflow-y:auto}
    @media(max-width:500px){.modal{border-radius:16px 16px 0 0;margin-top:auto}.overlay:not(.auth-modal-overlay){align-items:flex-end;padding:0}}
    ...
    .nav-tab.on{background:#0B1F35;color:#fff}
    .dark .nav-tab.on{background:#D4AF37;color:#061220}

    /* Topbar */
    .topbar{position:fixed;top:0;left:0;right:0;z-index:1000;height:68px;background:rgba(11,31,53,.96);backdrop-filter:blur(14px);border-bottom:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:space-between;padding:0 24px;gap:12px}
    ...
    .sch{flex:1;max-width:440px;position:relative}
    .sch input{width:100%;height:42px;padding:0 82px 0 42px;border-radius:30px;border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.07);color:#fff;font-size:14px;font-family:inherit;transition:all .2s}
    .sch input::placeholder{color:rgba(255,255,255,.4)}
    .sch input:focus{background:rgba(255,255,255,.12);border-color:#D4AF37;box-shadow:0 0 0 4px rgba(212,175,55,.15);outline:none}
    .sch .sch-icon{position:absolute;left:14px;top:50%;transform:translateY(-50%);color:rgba(255,255,255,.4);pointer-events:none}
    .sch .ask-btn{position:absolute;right:6px;top:50%;transform:translateY(-50%);height:30px;padding:0 10px;border-radius:20px;border:1.5px solid rgba(212,175,55,.6);background:rgba(212,175,55,.12);color:#D4AF37;font-size:12px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all .18s;white-space:nowrap;font-family:inherit}
    .sch .ask-btn:hover{background:rgba(212,175,55,.25);border-color:#D4AF37}
    ...
    .av:hover{border-color:#D4AF37}
    .create-btn{height:36px;padding:0 14px;border-radius:20px;background:rgba(255,255,255,.1);border:1.5px solid rgba(255,255,255,.2);color:#fff;font-size:13px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all .18s;white-space:nowrap;font-family:inherit;flex-shrink:0}
    .create-btn:hover{background:rgba(255,255,255,.18);border-color:rgba(255,255,255,.4)}
    ...
    .lsb-topics::-webkit-scrollbar-thumb{background:rgba(212,175,55,.3);border-radius:10px}
    ...
    /* Mobile-only category picker (appears when sidebar is hidden) */
    .mob-only-cat { display:none; }
    @media(max-width:1050px){ .mob-only-cat { display:block; } }

    /* (legacy mob-topics removed) */
    ...
    /* Staff card */
    .sc{background:var(--surf);border:1px solid var(--brd);border-radius:16px;overflow:hidden;text-align:center;transition:all .22s;cursor:pointer}
    .sc:hover{transform:translateY(-5px);box-shadow:0 12px 30px rgba(0,0,0,.12);border-color:#D4AF37}

    /* Star rating */
    .star{color:#D4AF37;font-size:18px}
    ...
    @media(max-width:700px){
      ...
      .sch{flex:1;max-width:none;min-width:0}
      .sch input{font-size:13px;height:36px;padding:0 52px 0 36px}
      .sch .ask-btn{right:5px;height:26px;padding:0 9px;gap:3px;font-size:11px;min-width:32px}
      .sch .ask-btn .ask-text{display:none}
      .hide-m{display:none !important}
      .create-btn .create-label{display:none}
      .create-btn{padding:0;width:36px;justify-content:center}
    }
    @media(max-width:500px){
      .modal{border-radius:16px 16px 0 0;margin-top:auto}
      .overlay{align-items:flex-end;padding:0}
      .overlay.auth-modal-overlay{align-items:center;padding:16px}
    }
    ...
    .fab{position:fixed;bottom:82px;right:20px;z-index:998;width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#D4AF37,#b89228);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 24px rgba(212,175,55,.45);animation:fabIn .4s cubic-bezier(.34,1.56,.64,1);transition:transform .18s,box-shadow .18s}
    .fab:hover{transform:scale(1.1);box-shadow:0 10px 32px rgba(212,175,55,.55)}
    ...
    .bm-active{animation:bookmarkPop .25s ease;color:#D4AF37 !important}
    ...
    .rxn.on{border-color:rgba(212,175,55,.5);background:rgba(212,175,55,.1);color:#D4AF37}
    ...
    /* ── AUTH MODAL ANIMATED BORDER ── */
    .auth-outer {
      position: relative;
      width: min(880px, 96vw);
      height: min(566px, calc(100dvh - 80px));
      background: repeating-conic-gradient(from var(--rot), #D4AF37 0%,#D4AF37 5%,transparent 5%,transparent 40%,#D4AF37 50%);
      border-radius: 32px;
    ...
    .auth-outer::before {
      ...
      background: repeating-conic-gradient(from var(--rot), #1A365D 0%,#1A365D 5%,transparent 5%,transparent 40%,#1A365D 50%);
      border-radius:32px;
      animation:authSpin 4s linear infinite;
      animation-delay:-1s;
    }
    ...
    .dark .auth-container { background:#0d1526; }

    /* ── Auth inner container (clips overflowing panels) ── */
    .auth-inner {
      position:absolute;inset:6px;border-radius:26px;
      overflow:hidden;background:var(--surf);
    }

    /* Form panels — positioned relative to .auth-inner */
    .auth-form-panel {
    ...
    .auth-toggle-box { position:absolute;width:100%;height:100%;overflow:hidden; }
    .auth-toggle-box::before {
      content:'';position:absolute;
      left:-250%;width:300%;height:100%;
      background:linear-gradient(135deg,#0B1F35 0%,#1A365D 50%,#b89228 150%);
    ...
    .cat-drop {
      position:absolute;top:calc(100% + 8px);bottom:auto;left:0;min-width:240px;
    ...
    .cat-drop-inner::-webkit-scrollbar-thumb{background:rgba(212,175,55,.3);border-radius:10px}
    .cat-item {
    ...
    .cat-item.on { background:rgba(212,175,55,.12);color:#D4AF37;font-weight:700; }
    ...
    .auth-inp:focus { background:var(--surf);box-shadow:0 0 0 2px rgba(212,175,55,.5); }
    .auth-inp::placeholder { color:var(--faint); }
    .auth-btn {
      width:100%;padding:13px;border-radius:9px;font-size:15px;font-weight:700;
      font-family:inherit;cursor:pointer;border:none;
      background:#0B1F35;color:#fff;transition:all .18s;
      display:flex;align-items:center;justify-content:center;gap:8px;
    }
    .auth-btn:hover { background:#1A365D;transform:translateY(-1px); }
    .auth-btn.gold-btn { background:linear-gradient(135deg,#D4AF37,#b89228);color:#061220; }
    .auth-btn.gold-btn:hover { box-shadow:0 6px 20px rgba(212,175,55,.4); }
    .auth-toggle-btn {
    ...
    .auth-social-btn:hover { border-color:#D4AF37;background:rgba(212,175,55,.08); }
    ...
    .apply-border-inner {
      position:relative;z-index:1;
      background:var(--surf);border-radius:19px;overflow:hidden;
    }

    /* ── MOBILE DRAWER ── */
    .show-m { display:none; }
    @media(max-width:700px){ .show-m { display:flex; } }
    .mob-drawer-overlay {
    ...
    .mob-drawer-item.on { color:#D4AF37;background:rgba(212,175,55,.08); }


    @media(max-width:380px){
      .auth-toggle-panel { padding:20px 16px; }
      .auth-toggle-panel h2 { font-size:18px !important; }
      .auth-toggle-panel p { font-size:12px !important; }
      .auth-form-panel { padding:16px 14px; }
    }
    @media(max-width:480px){
      .auth-outer { height:min(520px, calc(100dvh - 70px)); }
    }
    @media(max-width:650px){
      .auth-outer { width:min(96vw,460px);height:min(560px,calc(100dvh - 80px));border-radius:20px; }
      .auth-inner { inset:4px;border-radius:16px;overflow:hidden; }
      .auth-form-panel {
        width:100%;height:68%;
      ...
      .auth-outer.active .auth-form-panel { bottom:30%; }
      .auth-toggle-box::before {
        left:0;top:-270%;width:100%;height:300%;
        border-radius:20vw;
        transition:top 1.8s ease-in-out;
      }
      .auth-outer.active .auth-toggle-box::before { left:0;top:70%; }
      .auth-toggle-panel { width:100%;height:30%; }
      .auth-toggle-panel.auth-toggle-left { top:0;left:0;transition-delay:1.2s; }
      .auth-outer.active .auth-toggle-panel.auth-toggle-left { top:-30%;transition-delay:.6s; }
      .auth-toggle-panel.auth-toggle-right { right:0;top:auto;bottom:-30%;transition-delay:.6s; }
