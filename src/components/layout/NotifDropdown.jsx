...
function NotifDropdown({notifs,onMarkAll}) {
  ...
  return (
    <div style={{position:"fixed",top:68,right:16,width:"min(310px,calc(100vw - 32px))",maxWidth:310,background:"var(--surf)",border:"1px solid var(--brd)",borderRadius:16,boxShadow:"0 20px 40px rgba(0,0,0,.15)",zIndex:1200,overflow:"hidden",animation:"popIn .3s cubic-bezier(.34,1.56,.64,1)"}}>
      ...
      {notifs.map((n,i)=>(
        <div key={i} style={{padding:"14px 18px",display:"flex",gap:12,borderBottom:"1px solid var(--brd)",background:n.unread?"rgba(59,130,246,.06)":"transparent",cursor:"pointer",transition:"background .15s"}}
          onMouseEnter={e=>e.currentTarget.style.background="var(--surfH)"}
          onMouseLeave={e=>e.currentTarget.style.background=n.unread?"rgba(59,130,246,.06)":"transparent"}>
