...
const ToastContainer = ({toasts}) => (
  <div style={{position:"fixed",bottom:28,left:"50%",transform:"translateX(-50%)",zIndex:3000,display:"flex",flexDirection:"column",gap:10,alignItems:"center",pointerEvents:"none"}}>
    {toasts.map(t=>(
      <div key={t.id} className={`toast`} style={{background:t.type==="success"?"#10B981":t.type==="error"?"#EF4444":"#000B18",color:"#fff"}}>
...
const Stars = ({r=5})=>(
  <span>{[1,2,3,4,5].map(i=>(
    <span key={i} style={{color:i<=Math.round(r)?"#3B82F6":"#CBD5E1",fontSize:14}}>★</span>
  ))}</span>
);
...
function BackToTop({onAsk}) {
  ...
  return (
    <div style={{
      position:"fixed",bottom:90,right:18,zIndex:998,
      display:"flex",flexDirection:"column",alignItems:"center",gap:8,
    }}>
      {/* Ask FAB — always visible */}
      {onAsk && (
        <button onClick={onAsk} title="Ask a question"
          style={{
          ...
          onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.1)";e.currentTarget.style.boxShadow="0 10px 28px rgba(59,130,246,.55)";}}
          onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.boxShadow="0 6px 20px rgba(59,130,246,.45)";}}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#000B18" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
        </button>
      )}
      {/* Back to top — only when scrolled */}
      {show && (
        <button className="b2t" style={{position:"static",animation:"b2tIn .3s ease"}}
