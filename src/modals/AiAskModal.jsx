    ...
    <div className="overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="modal" style={{maxWidth:500}}>
        ...
        <div style={{padding:24,display:"flex",flexDirection:"column",gap:14}}>
          ...
          {ans&&<div style={{padding:16,borderRadius:12,background:"rgba(59,130,246,.08)",border:"1px solid rgba(59,130,246,.25)",fontSize:13,lineHeight:1.8,color:"var(--txt)",whiteSpace:"pre-line"}}>{ans.replace(/\*\*(.*?)\*\*/g,"$1")}</div>}
        </div>
      </div>
    </div>
  );
