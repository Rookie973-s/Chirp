      ...
      <div className="modal" style={{maxWidth:480,display:"flex",flexDirection:"column",height:"min(85vh, 600px)",maxHeight:"min(85vh, 600px)"}}>
        <div style={{background:`linear-gradient(135deg,${T.navy},${T.navyL})`,padding:"18px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:36,height:36,borderRadius:"50%",background:"rgba(212,175,55,.2)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          ...
          {msgs.map((m,i)=>(
            m.from==="system"?(
              ...
              <div key={i} style={{display:"flex",gap:8,flexDirection:m.self?"row-reverse":"row"}}>
                <div style={{width:30,height:30,borderRadius:"50%",background:m.bg,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:11,color:m.self?"#061220":"#fff",flexShrink:0}}>{m.av}</div>
                <div style={{maxWidth:"72%"}}>
                  {!m.self&&<div style={{fontSize:11,fontWeight:600,color:"var(--faint)",marginBottom:3}}>{m.from} · {m.time}</div>}
                  <div style={{padding:"10px 14px",borderRadius:m.self?"18px 18px 4px 18px":"18px 18px 18px 4px",background:m.self?"linear-gradient(135deg,#D4AF37,#b89228)":("var(--surf)"),color:m.self?"#061220":"var(--txt)",fontSize:13,lineHeight:1.5,border:m.self?"none":"1px solid var(--brd)"}}>{m.text}</div>
                </div>
              </div>
            )
          ))}
        ...
        <div style={{padding:"12px 16px",borderTop:"1px solid var(--brd)",display:"flex",gap:8,background:"var(--surf)",flexShrink:0}}>
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")send();}}
            ...
            disabled={!user}/>
          <button onClick={send} style={{width:40,height:40,borderRadius:"50%",background:"linear-gradient(135deg,#D4AF37,#b89228)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <Ic n="send" s={15} c="#061220"/>
          </button>
        </div>
      </div>
    </div>
