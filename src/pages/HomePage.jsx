...
function HomePage({onNav}) {
  ...
  return (
    <div>
      ...
      {/* Stats */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(70px,  1fr))",gap:16,  padding: 16, marginBottom:48}} className="fu fu1">
        {stats.map(([v,l])=>(
          <div key={l} className="card" style={{padding:"24px 16px",textAlign:"center"}}>
            <div style={{fontSize:30,fontWeight:700,fontFamily:"'Playfair Display',serif",color:T.gold,marginBottom:6}}>{v}</div>
      ...
      {/* Departments */}
      <h2 style={{fontSize:22,marginBottom:16,  padding: 16,}} className="fu fu3">Browse by Faculty</h2>
      <div style={{display:"flex",flexWrap:"wrap",gap:10,  padding: 16, marginBottom:48}} className="fu fu3">
        {DEPTS.map(d=>(
