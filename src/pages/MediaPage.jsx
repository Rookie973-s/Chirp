...
function MediaPage({user,onLogin,toast}) {
  ...
  return (
    <div style={{flex:1}}  className="media-wrap">
      ...
      <div style={{columns:"2 220px",gap:14}}>
        {MEDIA_POSTS.map((p,i)=>(
          <div key={p.id} className="card fu" style={{animationDelay:`${i*.07}s`,marginBottom:14,breakInside:"avoid",overflow:"hidden",cursor:"pointer"}}>
              ...
              {p.type==="video"&&(
                <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.25)"}}>
                  <div style={{width:44,height:44,borderRadius:"50%",background:"rgba(255,255,255,.85)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#0B1F35"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                  </div>
                </div>
              )}
            </div>
